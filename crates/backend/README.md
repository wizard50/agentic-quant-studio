...
| POST | `/studio/presentations` | Compile presentation dry-run — `{ graph }` → `PresentationSpec` (panes, outputs; no persist) |
...
# Dry-run presentation compile (no study created)
curl -s -X POST http://127.0.0.1:3000/api/v1/studio/presentations \
  -H "Content-Type: application/json" \
  -d '{
    "graph": {
      "id": "ds-sma",
  ...
  }' | jq '{version, panes: [.panes[].id], outputs}'
